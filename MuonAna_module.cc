/**********************************************************************************
 * Analyzer module created for single muon Kalman Filter direction reconstruction. 
 ...
 *  - EventID (int): ID of the event
    ...
    - MCParts (std::vector<int>): MC PDG number of all simulated particles
    - MCPartsX (std::vector<double>):MC X position of the origin of all 
      simulated particles (in cm, SBND coordinate system)
    - MCPartsY (std::vector<double>):MC Y position of the origin of all 
      simulated particles (in cm, SBND coordinate system)
    - MCPartsZ (std::vector<double>):MC Z position of the origin of all 
      simulated particles (in cm, SBND coordinate system)
    - MCPartsPx (std::vector<double> ): MC X component of the original 
      momentum of all simulated particles (GeV/c) 
    - MCPartsPy (std::vector<double>): MC Y component of the original 
      momentum of all simulated particles (GeV/c) 
    - MCPartsPz (std::vector<double>): MC Z component of the original 
      momentum of all simulated particles (GeV/c) 
    - MCPartsE (std::vector<double>): MC energy of the original 
      momentum of all simulated particles (GeV) 
    - MCPartsStatus (std::vector<int>): MC status of all the simulated 
...
void test::MuonAna::analyze(art::Event const& e)
{
  ...
  MCPartsPy.clear();
  MCPartsPz.clear();
  MCPartsFinalPx.clear();
  MCPartsFinalPy.clear();
  MCPartsFinalPz.clear();
  MCPartsE.clear();
  MCPartsStatus.clear();
...
//...............................
  ...
  //Monte-Carlo (true) particles information of the event. Here, information 
  ...
  for (auto const& truth : *mctruths) {
    ...
    for (int i=0; i<truth.NParticles(); i++){
      ...
      MCPartsE.push_back(particle.E());
      MCParts.push_back(particle.PdgCode());
      MCPartsPx.push_back(particle.Px());
      MCPartsPy.push_back(particle.Py());
      MCPartsPz.push_back(particle.Pz());
      MCPartsFinalPx.push_back(particle.EndPx());
      MCPartsFinalPy.push_back(particle.EndPy());
      MCPartsFinalPz.push_back(particle.EndPz());
      MCPartsX.push_back(particle.Vx());
      MCPartsY.push_back(particle.Vy());
      MCPartsZ.push_back(particle.Vz());
      MCPartsStatus.push_back(particle.StatusCode());
