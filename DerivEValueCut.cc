{
	/********************************************************************************************
	 * Macro that computes the reconstructed direction of the shower according to the derivative 
	 * eigenvalue cut. 
	...
	TH1D *h = new TH1D("d#lambda_{n}/dn < 0 cut", "#theta_{#hat{s}, e} with reco e direction (using reco points);#theta_{#hat{s}, e}(^{0});entries", 180, 0, 90);
	...
	for (i=0; i<t->GetEntries(); i++){
		...
		for (j=0; j<cumulEValue->size() - 1; j++){
			if (cumulEValue->at(j+1) - cumulEValue->at(j) < 0){
				nlastsegment = j;
				break;
			}
		}

		//Calculate theta (true, reco), with the direction reconstructed for the selected portion
	...
	std::cout<<"eigenvalue derivative < 0 cut in G4 EDepos reco info:"<<std::endl;
